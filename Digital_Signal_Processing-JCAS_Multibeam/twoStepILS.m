function [W0, Convergence_curve] = twoStepILS(iter_nr, alpha, V_pattern, W0, PM, PdM)
...
Convergence_curve = zeros(1, iter_nr);
...
for i = 1:iter_nr
    ...
    current_fitness = sum(abs(PM(:, alpha)-PdM(:, alpha)));
    Convergence_curve(i) = current_fitness;
    
    fprintf("Iteration %i, Total difference is %f \n", i, current_fitness);
    if isempty(K)
        % Fill remaining iterations with final value
        Convergence_curve(i:end) = current_fitness;
        break;
    end%if
